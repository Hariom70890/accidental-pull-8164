...
const ProductSummary = () => {
  ...
  const totalPrice = products.reduce((acc, product) => acc + product.data.price, 0);
...
useEffect(() => {
  ...
  return (
    <Box>
      ...
      <Heading as="h2" size="lg" mb={6}>Cart Items</Heading>
      <Grid templateColumns='repeat(2, 80% 20%)'>
      <Grid templateColumns='repeat(3, 1fr)'>
        {products.map((product) => (
          <Box key={product.data.id}>
            <Card imageUrl={product.data.imageAddress} name={product.data.name} price={product.data.price} />
          </Box>
        ))}
      </Grid>
        ...
        <Text>{products.length}</Text>
          ...
          <br />
          <Button backgroundColor={"green"} color={"whatsapp.100"} onClick={()=>{navigate("/paymentPage")}}>Process to Payment</Button>
        </Box>
    </Grid>
    </Box>
  );
